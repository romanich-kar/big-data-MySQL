...
-- представление для просмотра 10 самых свежих вакансий и 10 самых свежих резюме
CREATE OR REPLACE VIEW fresh AS
  SELECT id, emploer_id, description AS the_fresh_ones
  FROM vacancies
  ORDER BY created_at DESC
  LIMIT 10
  UNION
  SELECT id, emploee_id, description
  FROM cv
  ORDER BY created_at DESC
  LIMIT 10;
...
GRANT SELECT ON hh.users, hh.emploer, hh.fresh TO 'reader'@'localhost';